...
static CheckResult checkSpan(std::span<const double> s, unsigned expected_size,
                              const char* name)
{
    if (s.size() != expected_size)
        return { false, std::string(name) + ": size=" + std::to_string(s.size())
...
static CheckResult checkZeroFallback(std::span<const double> s, unsigned expected_size,
                                     const char* process_name, const char* variant_name)
{
...
static void printSpan(const char* label, std::span<const double> s,
                      bool is_zero_fallback, int col_width = 14)
{
    // Label + tag
...
static void printHeader(const char* variant_name, unsigned n_eq)
{
    std::cout << "\n";
    std::cout << "┌─────────────────────────────────────────────────";
    for (unsigned i = 0; i < n_eq; ++i) std::cout << "────────────────";
    std::cout << "┐\n";
    std::cout << "│  " << std::left << std::setw(40) << variant_name
              << "  n_equations = " << n_eq << std::string(8, ' ')
              << "│\n";
    std::cout << "├──────────────────────┬──────────────────────────";
    for (unsigned i = 0; i < n_eq; ++i) std::cout << "────────────────";
    std::cout << "┤\n";
                           ...
                           bool has_sintering)
{
    ...
    auto row = [&](const char* lbl, std::span<const double> s, bool zf) {
        std::cout << "│  " << std::left << std::setw(14) << lbl
                  << (zf ? " [ZF]" : "     ") << " │  ";
        std::cout << std::scientific << std::setprecision(4) << std::right;
        for (auto v : s) std::cout << std::setw(13) << v << "  ";
        std::cout << "│\n";
    };
    row("total",       src_all,  false);
    std::cout << "│                     ├──────────────────────────";
    for (unsigned i = 0; i < NEQ; ++i) std::cout << "────────────────";
    std::cout << "┤\n";
    ...
    for (auto [s, label] : std::initializer_list<std::pair<std::span<const double>, const char*>>{
            {src_all, "sources()"}, {src_nuc, "nucleation"}, {src_coa, "coagulation"},
            {src_con, "condensation"}, {src_gro, "growth"}, {src_oxi, "oxidation"},
            {src_sin, "sintering"} })
    {
        auto r = checkSpan(s, NEQ, label);
        if (!r.pass) { std::cout << "  [FAIL] " << r.detail << "\n"; ok = false; }
    }

    // ── Check 2: zero-fallback spans are exactly zero ─────────────────────
    auto zf = [&](std::span<const double> s, const char* proc, bool should_be_zero) {
        if (!should_be_zero) return;
        auto r = checkZeroFallback(s, NEQ, proc, name);
        if (!r.pass) { std::cout << "  [FAIL] " << r.detail << "\n"; ok = false; }
        else std::cout << "  [PASS] " << name << "::" << proc
    ...
    auto r3 = checkMassBalance(src_all,
                                { src_nuc, src_coa, src_con, src_gro, src_oxi, src_sin },
                                name);
    if (!r3.pass) { std::cout << "  [FAIL] " << r3.detail << "\n"; ok = false; }
    else std::cout << "  [PASS] " << name << ": sources() == sum of all per-process spans\n";

    // ── Check 4: at least one modelled process has non-zero value ─────────
    bool any_nonzero = false;
    for (auto [s, is_modelled] : std::initializer_list<std::pair<std::span<const double>, bool>>{
            {src_nuc, has_nucleation}, {src_coa, has_coagulation},
            {src_con, has_condensation}, {src_gro, has_growth},
            {src_oxi, has_oxidation}, {src_sin, has_sintering} })
    {
        if (is_modelled)
            any_nonzero |= std::any_of(s.begin(), s.end(),
...
template <typename Model>
constexpr bool hasNucleation()   { return requires(const Model& m){ m.sources_nucleation_impl(); }; }
template <typename Model>
constexpr bool hasCoagulation()  { return requires(const Model& m){ m.sources_coagulation_impl(); }; }
template <typename Model>
constexpr bool hasCondensation() { return requires(const Model& m){ m.sources_condensation_impl(); }; }
template <typename Model>
constexpr bool hasGrowth()       { return requires(const Model& m){ m.sources_growth_impl(); }; }
template <typename Model>
constexpr bool hasOxidation()    { return requires(const Model& m){ m.sources_oxidation_impl(); }; }
template <typename Model>
constexpr bool hasSintering()    { return requires(const Model& m){ m.sources_sintering_impl(); }; }
...
static void printOwnershipMatrix()
{
    ...
    static_assert( hasNucleation<H>()   && hasCoagulation<H>()  &&
                   hasCondensation<H>() && hasGrowth<H>()       &&
                   hasOxidation<H>()    && !hasSintering<H>(),
                   "HMOM ownership mismatch");
    // ThreeEquations: same as HMOM
    static_assert( hasNucleation<TE>()   && hasCoagulation<TE>()  &&
                   hasCondensation<TE>() && hasGrowth<TE>()       &&
                   hasOxidation<TE>()    && !hasSintering<TE>(),
                   "ThreeEquations ownership mismatch");
    // BrookesMoss: NO condensation, NO sintering
    static_assert( hasNucleation<BM>()    && hasCoagulation<BM>() &&
                   !hasCondensation<BM>() && hasGrowth<BM>()      &&
                   hasOxidation<BM>()     && !hasSintering<BM>(),
                   "BrookesMoss ownership mismatch");
    // TiO2: nucleation coagulation condensation sintering  NO growth NO oxidation
    static_assert( hasNucleation<T2>()   && hasCoagulation<T2>()  &&
                   hasCondensation<T2>() && !hasGrowth<T2>()      &&
                   !hasOxidation<T2>()   && hasSintering<T2>(),
    ...
    auto row = [&](const char* proc,
                   bool h, bool te, bool bm, bool t2) {
        std::cout << "│ " << std::left << std::setw(15) << proc << "│"
                  << (h  ? Y : N) << "  │"
                  << (te ? Y : N) << "    │"
                  << (bm ? Y : N) << "   │"
                  << (t2 ? Y : N) << " │\n";
    ...
    row("coagulation",  hasCoagulation<H>(),  hasCoagulation<TE>(),  hasCoagulation<BM>(),  hasCoagulation<T2>());
    row("condensation", hasCondensation<H>(), hasCondensation<TE>(), hasCondensation<BM>(), hasCondensation<T2>());
...
int main()
{
    ...
    std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    {
        ...
        std::cout << "  Moments: M00=" << ic[0] << "  M10=" << ic[1]
                  << "  M01=" << ic[2] << "  N0=" << ic[3] << "\n";

        all_ok &= verifyVariant(model, "HMOM",
                                /*nucleation*/   true,
                                /*coagulation*/  true,
                                /*condensation*/ true,
                                /*growth*/       true,
    ...
    std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    {
        ...
        all_ok &= verifyVariant(model, "ThreeEquations",
                                /*nucleation*/   true,
                                /*coagulation*/  true,
                                /*condensation*/ true,
                                /*growth*/       true,
    ...
    std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    {
        ...
        std::cout << "  State: T=" << T_soot << " K, P=" << P_atm << " Pa\n";
        std::cout << "  Moments: Ys=" << std::scientific << ic[0]*1e10
                  << "  bs=" << ic[1]*1e10 << "\n";

        all_ok &= verifyVariant(model, "BrookesMoss",
                                /*nucleation*/   true,
                                /*coagulation*/  true,
    ...
    std::cout << "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n";
    {
        ...
        all_ok &= verifyVariant(model, "TiO2",
                                /*nucleation*/   true,
                                /*coagulation*/  true,
                                /*condensation*/ true,
